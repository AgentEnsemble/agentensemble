...
- **Bounded iteration with `Loop`** -- declarative reflection, retry-until-valid, and debate patterns. A loop body of tasks repeats until a predicate fires or a `maxIterations` cap is hit; full per-iteration history lives in `EnsembleOutput.getLoopHistory(name)` and `ExecutionTrace.loopTraces`. Three memory modes including `WINDOW(n)` for prompt-size control. See the [Loops guide](https://agentensemble.net/guides/loops/).
- **State machines with `Graph`** -- LangGraph-equivalent state-machine flows for the JVM: named states (Tasks) connected by directed edges with conditional `GraphPredicate`s and arbitrary back-edges. Tool routers, selective-feedback edges, multi-turn negotiation. Per-step `onGraphStateCompleted` callbacks, full `getGraphHistory()` side channel, dedicated viz layout with conditional edge labels and fired/unfired styling. See the [Graphs guide](https://agentensemble.net/guides/graphs/).
