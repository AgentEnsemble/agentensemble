...
| `loops` | `List<Loop>` | Yes* | -- | Bounded iteration loops over a sub-ensemble of tasks. Add with `.loop(loop)`. Loops are first-class workflow nodes; in `SEQUENTIAL` workflow they run in declaration order after the task list, in `PARALLEL` they execute as nodes in the dependency DAG via the shadow-task pattern (concurrent with independent tasks; `Loop.context(...)` honoured for outer-DAG deps). Cannot be combined with `phases`; rejected with `Workflow.HIERARCHICAL`. See [Loops guide](../guides/loops.md). |
