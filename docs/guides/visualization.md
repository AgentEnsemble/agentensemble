...
## Live Mode (Coming in v2.1.0)

v2.1.0 will introduce a **Live Execution Dashboard** that turns the browser into a real-time
GUI for ensemble runs.

Instead of exporting a `.trace.json` file and opening it after the fact, the new
`agentensemble-web` module embeds a WebSocket server in your JVM process. As tasks start,
complete, and fail, events are pushed to the browser instantly. The timeline and flow graph
update live.

### Live streaming

```java
// Add agentensemble-web to your dependencies (v2.1.0+)
