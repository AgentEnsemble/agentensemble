 ...
 * listener lifecycle scoped to a single task.
 *
 * <p>The primary consumer is the core {@code TaskTraceAccumulator},
 * which forwards each event to the corresponding counter so that
 * {@code MemoryOperationCounts} is fully populated in the task trace.
 *
 * <p>This interface is activated when {@code CaptureMode.STANDARD}
 * or higher is in effect. At {@code CaptureMode.OFF}, no listener
...
public interface MemoryOperationListener {
     ...
     * Fired once per {@link MemoryContext#record(MemoryRecord)} call
